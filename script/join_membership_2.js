...
const msgErrorID = document.querySelector('#msgErrorID');
const msgErrorID2 = document.querySelector('#msgErrorID2');
...
userID.addEventListener('blur', function () {
  ...
  if (isID) {
    if (existID) {
      // 존재하는 아이디 처리
      msgErrorID2.classList.add('error');
    } else {
      //존재하지 않은 아이디 처리
      canSignUpID = true;
      msgErrorID2.classList.remove('error');
    }
  } else {
    // 잘못된 아이디 형식 처리
